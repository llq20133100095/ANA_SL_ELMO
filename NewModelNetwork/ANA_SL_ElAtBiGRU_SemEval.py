...
# -*- coding: utf-8 -*-
"""
Created on 2020.3.31 15:15

1.add the Elmo model in BiGRU
(1)loss: stimulative * entropy cross
(2)ANA
(3)elmo embedding + glove embedding
(4)adversarial training

2.add the comparision SL with entropy cross
@author: llq
"""
...
from SdpNetwork.CustomLoss import Stimulation_loss
from NewModelNetwork.dataElmo import ElmoData
from NewModelNetwork.CustomLayers import Input_keywords_DotLayer, concat_attention_layer, concat_attention_layer3, \
    concat_attention_layer3_1, concat_attention_layer3_2, concat_attention_layer3_3
...
class Network:
    ...
    def __init__(self):
        ...
        self.batch_size = 400
        # input shape
        self.input_shape = (None, 107, 1364)
        # mask shape
        ...
        self.save_picF1_path = "../result/f1-score.png"
        self.save_picAllAcc_path = "../result/test all accuracy.png"
        self.save_picAllRec_path = "../result/test all recall.png"
        ...
        """Self Attention"""
        self.input_shape_att = (None, 1364)
    ...
    def accuracy_f1(self, con_mat, support):
        ...
        for i in range(0, 18, 2):
            try:
                accuracy += float(con_mat[i][i] + con_mat[i + 1][i + 1]) / float(
                    np.sum(con_mat[:, i]) + np.sum(con_mat[:, i + 1]))
...
def train_run():
    ...
    elmo_data = ElmoData()
    elmo_data.dict_word2vec()

    # 2.traing_word_pos_vec3D:training data
    print("word_pos_vec3D.......")
    training_word_pos_vec3D, pos_vec, training_sen_length, train_sen_list2D = \
        elmo_data.embedding_lookup(elmo_data.train_sen_store_filename, \
                                   elmo_data.training_e1_e2_pos_filename, elmo_data.training_sen_number)

    training_word_pos_vec3D = elmo_data.merge_embedding(training_word_pos_vec3D, pos_vec, elmo_data.train_elmo_file)
    del pos_vec
    ...
    testing_word_pos_vec3D, pos_vec, testing_sen_length, test_sen_list2D = \
        elmo_data.embedding_lookup(elmo_data.test_sen_store_filename, \
                                   elmo_data.testing_e1_e2_pos_filename, elmo_data.testing_sen_number)
    testing_word_pos_vec3D = elmo_data.merge_embedding(testing_word_pos_vec3D, pos_vec, elmo_data.test_elmo_file)
    del pos_vec
    ...
    print("embedding root,e1 and e2.......")
    train_root_embedding, train_e1_embedding, train_e2_embedding = \
        elmo_data.embedding_looking_root_e1_e2(elmo_data.e1_sdp_train_file, \
                                               elmo_data.e2_sdp_train_file, elmo_data.training_sen_number,
                                               train_sen_list2D, elmo_data.train_elmo_file)

    test_root_embedding, test_e1_embedding, test_e2_embedding = \
        elmo_data.embedding_looking_root_e1_e2(elmo_data.e1_sdp_test_file, \
                                               elmo_data.e2_sdp_test_file, elmo_data.testing_sen_number,
                                               test_sen_list2D, elmo_data.test_elmo_file)
    ...
    #    train_keywords=train_keywords[indices]
    ...
    #    input_keywords=T.tensor3('input_keywords')
    ...
    ADAM
    """
    gru_network, l_in, l_mask, l_alphas, l_split_cnn = model.bulit_gru(input_var, mask_var, input_root, input_e1,
                                                                       input_e2)
    ...
    #    new_embedding_keywords=model.adversarial(loss, input_keywords)
    #    new_embedding_root=model.adversarial(loss,input_root)
    #    new_embedding_e1=model.adversarial(loss,input_e1)
    #    new_embedding_e2=model.adversarial(loss,input_e2)

    gru_network, l_in, l_mask, l_alphas, l_split_cnn = model.bulit_gru(new_embedding, mask_var, input_root, input_e1,
                                                                       input_e2)
    # mask_train_input: where "1" is pass. where "0" isn't pass.
    mask_train_input = pro_data.mask_train_input(training_label, num_labels=model.num_labels)
    prediction = lasagne.layers.get_output(gru_network)
    ...
    #    #focal loss
    #    loss_batch, _, _, _ = focal_loss(prediction, target_var)
    loss = T.mean(loss_batch, dtype=theano.config.floatX)
    ######################################
    ...
    if model.network_type == "pi":
        ...
        train_fn = theano.function(
            [input_var, target_var, mask_var, learning_rate_var, adam_beta1_var, negative_loss_alpha,
             negative_loss_lamda, input_root, input_e1, input_e2],
            [loss, train_acc, alpha, l_split, wrong_pre, true_pre, loss_batch, prediction_batch], updates=updates,
            on_unused_input='warn')
    #        train_fn = theano.function([input_var, target_var, mask_var, learning_rate_var, adam_beta1_var, negative_loss_alpha, negative_loss_lamda, input_keywords], [loss,train_acc,l_gru,l_split,wrong_pre,true_pre], updates=updates, on_unused_input='warn')
    ...
    val_fn = theano.function(
        ...
        [test_loss, test_acc, test_predicted_classid, alpha, loss_batch, prediction_batch], on_unused_input='warn')
    ...
    for epoch in range(model.num_epochs):
        ...
        if model.network_type == "pi":
            for batch in pro_data.iterate_minibatches_pi(training_word_pos_vec3D, \
                                                         training_label_1hot, training_sen_length, model.batch_size,
            ...
            for batch in pro_data.iterate_minibatches_inputAttRootE1E2(training_word_pos_vec3D, \
                                                                       ...
                                                                       shuffle=True):
                aa_inputs, targets, mask_sen_length, input_root_batch, input_e1_batch, input_e2_batch = batch
                aa_mark_input = model.mask(mask_sen_length, model.batch_size)

                err, acc, aa_l_gru, aa_l_split, wrong_pre, true_pre, loss_each_batch, predict_each_batch \
                    = train_fn(aa_inputs, targets, aa_mark_input, learning_rate, adam_beta1, model.negative_loss_alpha,
                               model.negative_loss_lamda, input_root_batch, input_e1_batch, input_e2_batch)
        ...
        for batch in pro_data.iterate_minibatches_inputAttRootE1E2(testing_word_pos_vec3D, \
            ...
            inputs, targets, mask_sen_length, input_root_batch, input_e1_batch, input_e2_batch = batch
            mark_input = model.mask(mask_sen_length, model.batch_size)
            err, acc, test_predicted_classid, alpha_list, loss_each_batch, predict_each_batch \
                = val_fn(inputs, targets, mark_input, model.negative_loss_alpha, model.negative_loss_lamda,
                         input_root_batch, input_e1_batch, input_e2_batch)
            test_err += err
            test_acc += acc
            test_batches += 1

            test_predicted_classi_all = np.concatenate((test_predicted_classi_all, test_predicted_classid))
            l_alpha_list.append(alpha_list)
        ...
        if (epoch % 10 == 0 and epoch != 0):
            ...
            model.save_plt(num_epochs, accuracy_listplt, 'test all accuracy', "Test All Accuracy", "accuracy(%)",
                           model.save_picAllAcc_path, showflag=False)
            # save Recall picture
            model.save_plt(num_epochs, recall_listplt, 'test all recall', "Test All Recall", "recall(%)",
                           model.save_picAllRec_path, showflag=False)
