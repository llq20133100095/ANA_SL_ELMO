...
from NewModelNetwork.ANA_SL_ElAtBiGRU_SemEval import Network
    ...
    for _ in range(3):
        ...
        if model.network_type == "pi":
            ...
            train_prediction_b = lasagne.layers.get_output(gru_network,
                                                           inputs={l_in: input_b_var,
                                                                   l_mask: mask_var})  # Second branch.
        ...
        gru_network, l_in, l_mask, l_alphas, l_split_cnn = model.bulit_gru(new_embedding, mask_var, input_root,
                                                                           input_e1,
                                                                           input_e2)
        # mask_train_input: where "1" is pass. where "0" isn't pass.
        mask_train_input = pro_data.mask_train_input(training_label, num_labels=model.num_labels)
        ...
        for epoch in range(model.num_epochs):
            ...
            if model.network_type == "pi":
                ...
                for batch in pro_data.iterate_minibatches_tempens(training_word_pos_vec3D, \
                    ...
                    err, acc, prediction = train_fn(aa_inputs, targets, aa_mark_input, z_targets, mask_train,
                                                    unsup_weight,
                                                    learning_rate, adam_beta1)
                ...
                for batch in pro_data.iterate_minibatches_inputAttRootE1E2(training_word_pos_vec3D, \
                    ...
                    err, acc, aa_l_gru, aa_l_split, wrong_pre, true_pre, loss_each_batch, predict_each_batch \
                        = train_fn(aa_inputs, targets, aa_mark_input, learning_rate, adam_beta1,
            ...
            for batch in pro_data.iterate_minibatches_inputAttRootE1E2(testing_word_pos_vec3D, \
                                                                       ...
                                                                       test_e1_embedding, test_e2_embedding,
                                                                       shuffle=False):
            ...
            if (epoch % 10 == 0 and epoch != 0):
                ...
                model.save_plt(num_epochs, f1_score_listplt, 'f1-score', "F1-SCORE", "f1-score(%)",
                               model.save_picF1_path,
                               showflag=False)
                ...
                model.save_plt(num_epochs, test_loss_listplt, 'test loss', "Test Loss", "loss",
                               model.save_lossTest_path,
                               showflag=False)
