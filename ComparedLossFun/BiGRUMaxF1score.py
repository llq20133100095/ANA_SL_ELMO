...
# -*- coding: utf-8 -*-
"""
Created on Wed Dec  5 11:41:00 2018

max f1 score loss function
...
from LossFunction import sga,max_f1_score_loss
...
#    loss,_,_,st_loss = focal_loss(prediction, target_var)
#    loss = lasagne.objectives.categorical_crossentropy(prediction, target_var)
    loss, N_d = max_f1_score_loss(prediction, target_var)
...
#    test_loss=lasagne.objectives.categorical_crossentropy(test_prediction, target_var)
    test_loss,_ = max_f1_score_loss(test_prediction,target_var)
    ...
    if model.network_type=="pi":
        ...
        train_fn = theano.function([input_var, target_var, mask_var, learning_rate_var, adam_beta1_var, negative_loss_alpha, negative_loss_lamda], [loss,train_acc,l_gru,l_split, N_d], updates=updates, on_unused_input='warn')
    
    # Compile a second function computing the validation loss and accuracy and F1-score:
    val_fn = theano.function([input_var, target_var, mask_var, negative_loss_alpha, negative_loss_lamda], [test_loss, test_acc, test_predicted_classid], on_unused_input='warn')
    ...
    for epoch in range(model.num_epochs):
        ...
        #Tempens model:run batch
            ...
            for batch in pro_data.iterate_minibatches(training_word_pos_vec3D, \
              training_label_1hot, training_sen_length, model.batch_size, shuffle=True):
                aa_inputs, targets, mask_sen_length = batch
                aa_mark_input=model.mask(mask_sen_length,model.batch_size)
                err ,acc, aa_l_gru,aa_l_split, N_d = train_fn(aa_inputs, targets, aa_mark_input, learning_rate, adam_beta1, model.negative_loss_alpha, model.negative_loss_lamda)
                train_err+=err
                train_acc+=acc
                train_batches += 1
