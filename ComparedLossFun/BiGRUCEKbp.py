...
@author: llq
@function:
    realise the cross entropy in BiGRU
"""
import numpy as np
import theano
...
if __name__ == "__main__":
    ...
    att1["Content-Disposition"] = 'attachment; filename="train_and_test_accuracy.png"'
    msg.attach(att1)
    ...
    # Construct attachment 3 and send the picture file in the current directory
    att1 = MIMEText(open(model.save_lr_path, 'rb').read(), 'base64', 'utf-8')
    att1["Content-Type"] = 'application/octet-stream'
    # filename in email display
    att1["Content-Disposition"] = 'attachment; filename="lr.png"'
    msg.attach(att1)
    try:
