...
# -*- coding: utf-8 -*-
"""
Created on Wed Apr 18 22:13:10 2018
...
class Network:
    ...
    def __init__(self):
        ...
        self.save_picAcc_path = "../result/RNN/train-test-accuracy.jpg"
        # save F1 picture path
        self.save_picF1_path = "../result/RNN/f1.jpg"
        self.save_picAllAcc_path = "../result/RNN/test all accuracy.jpg"
        self.save_picAllRec_path = "../result/RNN/test all recall.jpg"
        # save train loss picture path
        self.save_lossTrain_path = "../result/RNN/train-loss.jpg"
        # save test loss picture path
        self.save_lossTest_path = "../result/RNN/test-loss.jpg"
        # save result file
        self.save_result = "../result/RNN/result.txt"
        ...
        """ PR-data """
        self.pr_data = '../result/experiment_PR/RNN_conll04.npz'
    ...
    def bulit_gru(self, input_var=None, mask_var=None):
        ...
        l_rnn = lasagne.layers.RecurrentLayer(l_input_drop, num_units=self.gru_size,
                                              grad_clipping=self.grad_clip, only_return_final=False, backwards=False)

        l_rnn = lasagne.layers.RecurrentLayer(l_rnn, num_units=self.gru_size,
                                              grad_clipping=self.grad_clip, only_return_final=True, backwards=False)

        l_rnn_backward = lasagne.layers.RecurrentLayer(l_input_drop, num_units=self.gru_size,
                                                       grad_clipping=self.grad_clip, only_return_final=False,
                                                       backwards=True)

        l_rnn_backward = lasagne.layers.RecurrentLayer(l_rnn_backward, num_units=self.gru_size,
                                                       grad_clipping=self.grad_clip, only_return_final=True,
                                                       backwards=True)

        # Merge forward layers and backward layers
        l_merge = lasagne.layers.ElemwiseSumLayer([l_rnn, l_rnn_backward])
...
if __name__ == "__main__":
    ...
    for epoch in range(model.num_epochs):
        ...
        #                                           testing_label_1hot, testing_sen_length, model.batch_size,
        #                                           shuffle=False):
