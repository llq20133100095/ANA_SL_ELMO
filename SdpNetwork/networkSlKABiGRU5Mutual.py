...
# -*- coding: utf-8 -*-
"""
Created on Mon May 31 17:12:09 2018
...
from CustomLoss import SSL_Mutual,kl_loss_compute
...
#    loss = Negative_loss(prediction, target_var, negative_loss_alpha, negative_loss_lamda)
    loss,beta_false,beta_score = SSL_Mutual(prediction, target_var)
    loss2,_,_ = SSL_Mutual(prediction2, target_var)
...
#        loss=loss+pi_loss
    ...
    #KL-LOSS
    loss=loss+kl_loss_compute(prediction,prediction2,target_var)
    loss2=loss2+kl_loss_compute(prediction2,prediction,target_var)
...
#    test_loss=lasagne.objectives.categorical_crossentropy(test_prediction, target_var)
    test_loss,_,_ = SSL_Mutual(test_prediction,target_var)
    test_loss = T.mean(test_loss,dtype=theano.config.floatX)
    test_loss2,_,_ = SSL_Mutual(test_prediction2,target_var)
    ...
    if model.network_type=="pi":
        ...
        train_fn = theano.function([input_var, target_var, mask_var, learning_rate_var, adam_beta1_var, negative_loss_alpha, negative_loss_lamda, input_root, input_e1, input_e2], [loss,train_acc,beta_false,beta_score], updates=updates, on_unused_input='warn')
        train_fn2 = theano.function([input_var, target_var, mask_var, learning_rate_var, adam_beta1_var, negative_loss_alpha, negative_loss_lamda, input_root, input_e1, input_e2], [loss2,train_acc2], updates=updates2, on_unused_input='warn')
...
#    beta_score_listepoch=[]
#    beta_false_listepoch=[]
        ...
        #Tempens model:run batch
            ...
            for batch in pro_data.iterate_minibatches_inputAttRootE1E2(training_word_pos_vec3D, \
              training_label_1hot, training_sen_length, model.batch_size, train_root_embedding, train_e1_embedding, train_e2_embedding, shuffle=True):
                ...
                aa_mark_input=model.mask(mask_sen_length,model.batch_size)
                err ,acc, beta_false,beta_score = train_fn(aa_inputs, targets, aa_mark_input, learning_rate, adam_beta1, model.negative_loss_alpha, model.negative_loss_lamda, input_root, input_e1, input_e2)                
                err2 ,acc2 = train_fn2(aa_inputs, targets, aa_mark_input, learning_rate, adam_beta1, model.negative_loss_alpha, model.negative_loss_lamda, input_root, input_e1, input_e2)                
                
#                beta_score_list.append(beta_score)
#                beta_false_list.append(beta_false)
                train_err+=err
                train_acc+=acc
                train_err2+=err2
                train_acc2+=acc2
...
#        beta_score_listepoch.append(beta_score_list)
#        beta_false_listepoch.append(beta_false_list)
        #train accuracy
