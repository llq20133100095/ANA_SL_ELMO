...
# -*- coding: utf-8 -*-
"""
Created on Sat Mar  3 21:10:04 2018

output attention entity and root + negative loss + ensemble
...
#        loss=loss+pi_loss
    ...
    #calculate the mode
    test_classid=T.concatenate((T.reshape(test_predicted_classid,(-1,1)),T.reshape(test_predicted_classid2,(-1,1)),T.reshape(test_predicted_classid3,(-1,1)),T.reshape(test_predicted_classid4,(-1,1)),T.reshape(test_predicted_classid5,(-1,1))),axis=1)
    ...
    test_acc6 = T.mean(T.eq(test_classid, T.argmax(target_var, axis=1)),
                      dtype=theano.config.floatX)
    ...
    final = theano.function([input_var, target_var, mask_var, negative_loss_alpha, negative_loss_lamda, input_root, input_e1, input_e2, sen_length], [test_acc6,test_classid], on_unused_input='warn')
    ...
    for epoch in range(model.num_epochs):
        ...
        for batch in pro_data.iterate_minibatches_inputAttRootE1E2(testing_word_pos_vec3D, \
          testing_label_1hot, testing_sen_length, model.batch_size, test_root_embedding, test_e1_embedding, test_e2_embedding, shuffle=False):
            ...
            #sencond model
            err4, acc4, test_predicted_classid4 = val_fn2(inputs, targets, mark_input, model.negative_loss_alpha, model.negative_loss_lamda, input_root, input_e1, input_e2, np.reshape(mask_sen_length,(-1,1)))
            test_err4 += err4
            test_acc4 += acc4
            
            #third model
            err5, acc5, test_predicted_classid5 = val_fn3(inputs, targets, mark_input, model.negative_loss_alpha, model.negative_loss_lamda, input_root, input_e1, input_e2, np.reshape(mask_sen_length,(-1,1)))
            test_err5 += err5
            test_acc5 += acc5
