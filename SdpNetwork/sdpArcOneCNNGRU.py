...
# -*- coding: utf-8 -*-
"""
Created on Fri Feb  2 15:20:40 2018
...
from newLoss.lossFunction import categorical_crossentropy_margin,focal_loss
...
class Network:
    ...
    def bulit_gru(self,input_var=None,mask_var=None, left_sdp_length=None, sen_length=None, f3_depth=2):
        """
        ...
        l_global_sdp_cnn_3f = lasagne.layers.Conv2DLayer(
            l_split_global_sdp, num_filters=200, filter_size=(3, self.cnn_gru_size),
        ...
        # Convolutional layer in global SDP
        l_global_sdp_cnn_3f = lasagne.layers.Conv2DLayer(
            l_global_sdp_pooling_3f, num_filters=200, filter_size=(3, 1),
            nonlinearity=lasagne.nonlinearities.rectify,
            W=lasagne.init.GlorotUniform())
        ...
        l_cnn_fore_2f = lasagne.layers.Conv2DLayer(
            l_split_global_sdp, num_filters=200, filter_size=(2, self.cnn_gru_size),
            nonlinearity=lasagne.nonlinearities.rectify,
            W=lasagne.init.GlorotUniform())
        ...
        # Convolutional layer in global SDP
        l_cnn_fore_2f = lasagne.layers.Conv2DLayer(
            l_pooling_fore_2f, num_filters=200, filter_size=(2, 1),
            nonlinearity=lasagne.nonlinearities.rectify,
            W=lasagne.init.GlorotUniform())
        ...
        l_cnn_back_2f = lasagne.layers.Conv2DLayer(
            l_sdp_back, num_filters=200, filter_size=(2, self.cnn_gru_size),
            nonlinearity=lasagne.nonlinearities.rectify,
            W=lasagne.init.GlorotUniform())
        ...
        # Convolutional layer in global SDP
        l_cnn_back_2f = lasagne.layers.Conv2DLayer(
            l_pooling_back_2f, num_filters=200, filter_size=(2, 1),
            nonlinearity=lasagne.nonlinearities.rectify,
            W=lasagne.init.GlorotUniform())
        ...
        return l_out,l_in,l_mask,l_cnn_back_2f,l_con_den
...
#    training_label_1hot=training_label_1hot[indices]
    ...
    if model.network_type=="pi":
        ...
        train_fn = theano.function([input_var, target_var, mask_var, learning_rate_var, adam_beta1_var, left_sdp_length, sen_length], [loss,train_acc,l_gru,l_split], updates=updates, on_unused_input='warn')
    
    # Compile a second function computing the validation loss and accuracy and F1-score:
    val_fn = theano.function([input_var, target_var, mask_var, left_sdp_length, sen_length], [test_loss, test_acc, test_predicted_classid], on_unused_input='warn')
    ...
    for epoch in range(model.num_epochs):
        ...
        #Tempens model:run batch
            ...
            for batch in sdp_pro.iterate_minibatches(training_word_pos_vec3D, \
              training_label, training_sen_length, train_left_sdp_length, model.batch_size, shuffle=True):
                aa_inputs, targets, mask_sen_length, aa_left_sdp_length = batch
                aa_mark_input=model.mask(mask_sen_length,model.batch_size)
                err ,acc, aa_l_gru,aa_l_split = train_fn(aa_inputs, targets, aa_mark_input, learning_rate, adam_beta1, aa_left_sdp_length, np.reshape(mask_sen_length,(-1,1)))
                train_err+=err
                train_acc+=acc
                train_batches += 1
        ...
        for batch in sdp_pro.iterate_minibatches(testing_word_pos_vec3D, \
          testing_label, testing_sen_length, test_left_sdp_length, model.batch_size, shuffle=False):
            inputs, targets, mask_sen_length, left_sdp_length = batch
            mark_input=model.mask(mask_sen_length,model.batch_size)
            err, acc, test_predicted_classid = val_fn(inputs, targets, mark_input, left_sdp_length, np.reshape(mask_sen_length,(-1,1)))
            test_err += err
            test_acc += acc
            test_batches += 1
        ...
        err, acc, test_predicted_classid=val_fn(testing_word_pos_vec3D, testing_label, mark_input, test_left_sdp_length, np.reshape(testing_sen_length,(-1,1)))
        #confusion matrix
