...
# -*- coding: utf-8 -*-
"""
Created on Thu May 31 08:18:46 2018

auxiliary classifiers+KAS+SSL
...
from CustomLoss import Margin_loss
from piGRU.DotSofTraLayers import SelfAttEntRootLayer,SelfAttEntRootLayer2,SelfAttEntRootLayer3,SelfAttEntRootLayerVariant
...
class Network:
    ...
    def __init__(self):
        ...
        self.cell_parameters = lasagne.layers.recurrent.Gate(
            ...
            W_cell=lasagne.init.Normal(0.1), b=lasagne.init.Constant(0.),
...
##        l_kas=SelfAttEntRootLayer3((l_self_att,l_merge))
        ...
        """Aux"""
        l_merge_fc_aux = lasagne.layers.batch_norm(lasagne.layers.DenseLayer(
            l_merge,
            num_units=250,
            W=lasagne.init.GlorotUniform(gain=1.0), b=lasagne.init.Constant(1.),
            nonlinearity=lasagne.nonlinearities.selu))
  
#        l_out_margin_aux = lasagne.layers.DenseLayer(\
#            l_merge_fc_aux, num_units=self.num_classes,\
#            nonlinearity=lasagne.nonlinearities.softmax)
            
        #Margin loss
        #init w_classes: it's format is (class_number,network_output[1])
        w_classes_init=np.sqrt(6.0/(19+200))
        l_out_margin_aux=MarginLossLayer(l_merge_fc_aux,w_classes=lasagne.init.Uniform(w_classes_init), class_number=19)

        
        return l_out_margin,l_in,l_mask,l_out_margin,l_out_margin_aux
...
if __name__=="__main__":
    ...
    ADAM
    """
    gru_network,l_in,l_mask,l_gru_forward,l_out_margin_aux=model.bulit_gru(input_var,mask_var,input_root,input_e1,input_e2)
    ...
    prediction_aux = lasagne.layers.get_output(l_out_margin_aux)
...
#    loss_aux = lasagne.objectives.categorical_crossentropy(prediction_aux, target_var)
    loss_aux,_,_ = Margin_loss(prediction_aux, target_var)
...
#        loss=loss+pi_loss
        ...
        loss_aux = T.mean(loss_aux, dtype=theano.config.floatX)
        
    #regularization:L1,L2
    l2_penalty = lasagne.regularization.regularize_network_params(gru_network, lasagne.regularization.l2) * model.l2_loss
    loss=loss+l2_penalty
    l2_penalty_aux = lasagne.regularization.regularize_network_params(l_out_margin_aux, lasagne.regularization.l2) * model.l2_loss
    loss_aux=loss_aux+l2_penalty_aux    
    
    train_acc = T.mean(T.eq(T.argmax(prediction, axis=1), T.argmax(target_var, axis=1)),
                      dtype=theano.config.floatX)
    train_acc_aux = T.mean(T.eq(T.argmax(prediction_aux, axis=1), T.argmax(target_var, axis=1)),
                      dtype=theano.config.floatX)
    ...
    params_aux = lasagne.layers.get_all_params(l_out_margin_aux, trainable=True)
    updates_aux = lasagne.updates.adam(
            loss_aux, params_aux, learning_rate=learning_rate_var, beta1=adam_beta1_var)
...
#            loss, params, learning_rate=learning_rate_var)
    ...
    test_prediction_aux = lasagne.layers.get_output(l_out_margin_aux, deterministic=True)
...
#    test_loss=lasagne.objectives.categorical_crossentropy(test_prediction, target_var)
    ...
    test_predicted_classid_aux=T.argmax(test_prediction_aux, axis=1)
    ...
    if model.network_type=="pi":
        ...
        train_fn = theano.function([input_var, target_var, mask_var, learning_rate_var, adam_beta1_var, negative_loss_alpha, negative_loss_lamda, input_root, input_e1, input_e2], [loss,train_acc,l_gru,prediction_aux], updates=updates, on_unused_input='warn')
        train_fn_aux = theano.function([input_var, target_var, mask_var, learning_rate_var, adam_beta1_var, negative_loss_alpha, negative_loss_lamda, input_root, input_e1, input_e2], [loss_aux,train_acc_aux,l_gru,prediction_aux], updates=updates_aux, on_unused_input='warn')

    # Compile a second function computing the validation loss and accuracy and F1-score:
    val_fn = theano.function([input_var, target_var, mask_var, negative_loss_alpha, negative_loss_lamda, input_root, input_e1, input_e2], [test_loss, test_acc, test_predicted_classid,l_gru], on_unused_input='warn')
    val_fn_aux = theano.function([input_var, target_var, mask_var, negative_loss_alpha, negative_loss_lamda, input_root, input_e1, input_e2], [test_loss, test_acc, test_predicted_classid_aux,l_gru], on_unused_input='warn')
        ...
        #Tempens model:run batch
            ...
            for batch in pro_data.iterate_minibatches_inputAttRootE1E2(training_word_pos_vec3D, \
              training_label_1hot, training_sen_length, model.batch_size, train_root_embedding, train_e1_embedding, train_e2_embedding, shuffle=True):
                ...
                aa_mark_input=model.mask(mask_sen_length,model.batch_size)
#                if(epoch<50):
                err ,acc, aa_l_gru,aa_l_split = train_fn(aa_inputs, targets, aa_mark_input, learning_rate, adam_beta1, model.negative_loss_alpha, model.negative_loss_lamda, input_root, input_e1, input_e2)                
                err_aux ,acc_aux, _,_ = train_fn_aux(aa_inputs, targets, aa_mark_input, learning_rate, adam_beta1, model.negative_loss_alpha, model.negative_loss_lamda, input_root, input_e1, input_e2)
#                else:
#                    err ,acc, aa_l_gru,aa_l_split = train_fn_sgd(aa_inputs, targets, aa_mark_input, learning_rate, adam_beta1, model.negative_loss_alpha, model.negative_loss_lamda, input_root, input_e1, input_e2)
                train_err+=err
                train_err_aux+=err_aux
                train_acc+=acc
                train_acc_aux+=acc_aux
                train_batches += 1
        ...
        test_predicted_classi_all_aux=np.array([0])
        for batch in pro_data.iterate_minibatches_inputAttRootE1E2(testing_word_pos_vec3D, \
          testing_label_1hot, testing_sen_length, model.batch_size, test_root_embedding, test_e1_embedding, test_e2_embedding, shuffle=False):
            ...
            mark_input=model.mask(mask_sen_length,model.batch_size)
            err, acc, test_predicted_classid, _ = val_fn(inputs, targets, mark_input, model.negative_loss_alpha, model.negative_loss_lamda, input_root, input_e1, input_e2)
            _, _, test_predicted_classid_aux, _ = val_fn_aux(inputs, targets, mark_input, model.negative_loss_alpha, model.negative_loss_lamda, input_root, input_e1, input_e2)
            ...
            test_predicted_classi_all_aux=np.concatenate((test_predicted_classi_all_aux,test_predicted_classid_aux))
        
        test_predicted_classid=np.array(test_predicted_classi_all[1:-83])
        test_predicted_classid_aux=np.array(test_predicted_classi_all_aux[1:-83])
...
#        _, _, test_predicted_classid_aux,_=val_fn_aux(testing_word_pos_vec3D, testing_label_1hot, mark_input, model.negative_loss_alpha, model.negative_loss_lamda, test_root_embedding, test_e1_embedding, test_e2_embedding)
        ...
        #confusion matrix
        test_con_mat=confusion_matrix(testing_label[:-83],test_predicted_classid_aux)
        #support
        _,_,_,support=precision_recall_fscore_support(testing_label[:-83],test_predicted_classid_aux)   
        #computer F1_Score
        test_f1_score_aux,_,_=model.accuracy_f1(test_con_mat,support)
        
        f1_score_listplt.append(test_f1_score*100)
        f1_score_listplt_aux.append(test_f1_score_aux*100)
        ...
        #max f1-score-aux
        if(f1_score_max_aux<test_f1_score_aux):
            f1_score_max_aux=test_f1_score_aux
        ...
        print("  training loss aux:\t\t{:.6f}".format(train_err_aux / train_batches))
        print("  training accuracy:\t\t{:.2f} %".format(train_acc / train_batches * 100))
        print("  training accuracy aux:\t{:.2f} %".format(train_acc_aux / train_batches * 100))
        ...
        print("  max  f1 score aux:\t\t{:.2f} %".format(f1_score_max_aux*100))
        ...
        email_content=email_content+"  max  f1 score aux:\t\t{:.2f} %".format(f1_score_max_aux*100)+"\n"
...
#                learning_rate=learning_rate/2.0
    ...
    np.savez('../model/model_aux.npz', *lasagne.layers.get_all_param_values(gru_network))
    
#    model_save=Network()
#    gru_network_save,_,_,_,_,_=model_save.bulit_gru(input_var,mask_var,input_att_var,mask_att_train)
#    #And load them again later on like this:
#    with np.load('../model/model.npz') as f:
#         param_values = [f['arr_%d' % i] for i in range(len(f.files))]
#    lasagne.layers.set_all_param_values(gru_network_save, param_values)
