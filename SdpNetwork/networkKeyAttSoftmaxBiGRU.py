...
# -*- coding: utf-8 -*-
"""
Created on Sun Feb 11 11:06:13 2018

softmax loss+ keywords-attention
...
#    #gradient
#    grads_w_classes = T.grad(loss, gru_network.w_classes)
#    grads_u_root = T.grad(loss, l_gru_forward.u_e1)
#    grads_resetgate = T.grad(loss, l_split_cnn.W_in_to_updategate)
...
#        train_fn = theano.function([input_var, target_var, mask_var, learning_rate_var, adam_beta1_var, negative_loss_alpha, negative_loss_lamda, input_root, input_e1, input_e2, sen_length], [loss,train_acc,prediction,l_split,grads_w_classes,grads_u_root,grads_resetgate,coding_dist,coding_dist_true2inf], updates=updates, on_unused_input='warn')
        train_fn = theano.function([input_var, target_var, mask_var, learning_rate_var, adam_beta1_var, negative_loss_alpha, negative_loss_lamda, input_root, input_e1, input_e2, sen_length], [loss,train_acc,l_extra_loss,l_split], updates=updates, on_unused_input='warn')
    ...
    for epoch in range(model.num_epochs):
        ...
        #Tempens model:run batch
            ...
            #Ordinary model
            grads_w_classes_list_epoch=[]
            grads_root_list_epoch=[]
            grads_gate_list_epoch=[]
            aa_l_gru_list_epoch=[]
            ...
            for batch in pro_data.iterate_minibatches_inputAttRootE1E2(training_word_pos_vec3D, \
              training_label_1hot, training_sen_length, model.batch_size, train_root_embedding, train_e1_embedding, train_e2_embedding, shuffle=True):
                ...
                aa_mark_input=model.mask(mask_sen_length,model.batch_size)
#                err ,acc, aa_l_gru,aa_l_split,grads_w_classes,grads_root,grads_gate,coding_dist,coding_dist_true2inf = train_fn(aa_inputs, targets, aa_mark_input, learning_rate, adam_beta1, model.negative_loss_alpha, model.negative_loss_lamda, input_root, input_e1, input_e2, np.reshape(mask_sen_length,(-1,1)))
                err ,acc, aa_l_gru,aa_l_split = train_fn(aa_inputs, targets, aa_mark_input, learning_rate, adam_beta1, model.negative_loss_alpha, model.negative_loss_lamda, input_root, input_e1, input_e2, np.reshape(mask_sen_length,(-1,1)))
    
                train_err+=err
                train_acc+=acc
                train_batches += 1
#                grads_w_classes_list_epoch.append(grads_w_classes)
#                grads_root_list_epoch.append(grads_root)
#                grads_gate_list_epoch.append(grads_gate)
#                
#                aa_l_gru_list_epoch.append(coding_dist)
#                #second big  
#                coding_dist_true2inf_list_epoch.append(coding_dist_true2inf)
                
            grads_w_classes_list.append(grads_w_classes_list_epoch)
            grads_root_list.append(grads_root_list_epoch)
            grads_gate_list.append(grads_gate_list_epoch)
