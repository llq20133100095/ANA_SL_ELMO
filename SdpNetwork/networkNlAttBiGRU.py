...
# -*- coding: utf-8 -*-
"""
Created on Thu Feb  8 09:22:50 2018

New loss+Attention+BiGru
...
from piGRU.DotSofTraLayers import AttentionLayer,InputAttentionLayer
...
class Network:
    ...
    def __init__(self):
        ...
        #input shape
        self.input_shape=(None,106,340)
        #mask shape
        self.mask_shape=(None,106)
        ...
        self.scaled_unsup_weight_max=2.0
        ...
        """Entity Attention"""
        #the number of unrolled steps of ATT-GRU
        self.att_num_steps=6
        #dropout probability
        self.keep_prob_att_input=0.2
        #input entity shape
        self.input_entity_shape=(None,6,300)
        #mask entity shape
        self.mask_entity_shape=(None,6)
        
    def input_att_gru(self,input_entity_var=None,mask_entity_var=None):
        """
        Entity Attention Gru Model
        """
        #input layer
        l_in_att=lasagne.layers.InputLayer(shape=self.input_entity_shape,input_var=input_entity_var,name="l_in_att")
        
#        #mask layer
#        l_mask_att=lasagne.layers.InputLayer(shape=self.mask_entity_shape,input_var=mask_entity_var,name="l_mask_att")
        
        #Reshape
        l_in_att=lasagne.layers.ReshapeLayer(l_in_att,([0],1,[1],[2]))
        
        # Convolutional layer in global SDP
        l_att_cnn = lasagne.layers.Conv2DLayer(
            l_in_att, num_filters=340, filter_size=(2, 300),
            nonlinearity=lasagne.nonlinearities.rectify,
            W=lasagne.init.GlorotUniform())
                
        # MAX-pooling in global SDP
        l_att_maxpooling=lasagne.layers.GlobalPoolLayer(l_att_cnn, pool_function=T.max)

#        #dropout
#        l_drop_att=lasagne.layers.DropoutLayer(l_in_att,p=self.keep_prob_input)
#        
#        #GRU forward
#        l_input_att=lasagne.layers.LSTMLayer(\
#            l_drop_att,num_units=340,grad_clipping=self.grad_clip,
#            ingate=self.gate_parameters, forgetgate=self.gate_parameters,
#            cell=self.cell_parameters, outgate=self.gate_parameters,
#            learn_init=True,only_return_final=True,name="l_lstm_forward_att")
        
        return l_att_maxpooling

    def bulit_gru(self,input_var=None,mask_var=None,input_entity_var=None,mask_entity_var=None):
        """
        ...
        """Input attention"""
#        l_input_att=self.input_att_gru(input_entity_var,mask_entity_var)
        
        #Attention layer
        l_att=InputAttentionLayer(l_in,name="attention")
        ...
        #inpute dropout
        l_input_drop=lasagne.layers.DropoutLayer(l_att,p=self.keep_prob_input)
       ...
       #Two GRU forward
        ...
        l_gru_backward=lasagne.layers.GRULayer(\
            ...
            resetgate=self.gate_parameters,updategate=self.gate_parameters,
            hidden_update=self.cell_parameters,learn_init=True,
            only_return_final=False,backwards=False,name="l_gru_backward1")
        ...
        return l_out_margin,l_in,l_mask,l_att,l_out_margin
    ...
    def mask_att(self,mask_var,batch_size):
        """
        Attention mask input:it's size is (batch_size,6)
        When mask[i,j]=1,so it can input.
        When mask[i,j]=0,so it can't input.
        """
        mark_input=np.int32(np.zeros((batch_size,self.att_num_steps)))
        sentence=0
        for length in mask_var:
            mark_input[sentence]=np.concatenate((np.ones((1,length)),np.zeros((1,self.att_num_steps-length))),axis=1)
            sentence=sentence+1
        return mark_input
...
if __name__=="__main__":
    ...
    pro_data.training_word_pos_vec3D,pro_data.training_sen_length=\
    ...
    pro_data.testing_word_pos_vec3D,pro_data.testing_sen_length=pro_data.embedding_lookup(pro_data.testing_word_vec3D,\
    ...
    #8.get entity pair and embedding them
    pro_data.entity_pair_train3D,pro_data.entity_train_len=pro_data.entity_embedding(pro_data.entity_train_file,pro_data.entity_pair_train3D)
    entity_pair_train3D=np.float32(pro_data.entity_pair_train3D)
    entity_train_len=np.int32(pro_data.entity_train_len)
    
    pro_data.entity_pair_test3D,pro_data.entity_test_len=pro_data.entity_embedding(pro_data.entity_test_file,pro_data.entity_pair_test3D)
    entity_pair_test3D=np.float32(pro_data.entity_pair_test3D)
    entity_test_len=np.int32(pro_data.entity_test_len)
    ...
    entity_pair_train3D=entity_pair_train3D[indices]
    entity_train_len=entity_train_len[indices]
    ...
    #input_attention variables:
    input_att_var=T.tensor3('inputs_atten')
    mask_att_train=T.imatrix('mask_att_train')
...
#    sen_length=T.imatrix('sen_length')
    ...
    ADAM
    """
    gru_network,l_in,l_mask,l_gru_forward,l_split_cnn=model.bulit_gru(input_var,mask_var,input_att_var,mask_att_train)
...
#    test_loss = Negative_loss(test_prediction,target_var,negative_loss_alpha,negative_loss_lamda)
    ...
    if model.network_type=="pi":
        ...
        train_fn = theano.function([input_var, target_var, mask_var, learning_rate_var, adam_beta1_var, negative_loss_alpha, negative_loss_lamda, input_att_var, mask_att_train], [loss,train_acc,l_gru,l_split], updates=updates, on_unused_input='warn')
    
    # Compile a second function computing the validation loss and accuracy and F1-score:
    val_fn = theano.function([input_var, target_var, mask_var, negative_loss_alpha, negative_loss_lamda, input_att_var, mask_att_train], [test_loss, test_acc, test_predicted_classid], on_unused_input='warn')
    ...
    for epoch in range(model.num_epochs):
        ...
        #Tempens model:run batch
            ...
            #Ordinary model
            for batch in pro_data.iterate_minibatches(training_word_pos_vec3D, \
              training_label_1hot, training_sen_length, model.batch_size, entity_pair_train3D, entity_train_len, shuffle=True):
                aa_inputs, targets, mask_sen_length, input_att, input_att_mask_gru = batch
                aa_mark_input=model.mask(mask_sen_length,model.batch_size)
                #attention mask gru(when=="1",and the gru can get through)
                att_mask_input=model.mask_att(input_att_mask_gru,model.batch_size)
                
                err ,acc, aa_l_gru,aa_l_split = train_fn(aa_inputs, targets, aa_mark_input, learning_rate, adam_beta1, model.negative_loss_alpha, model.negative_loss_lamda,input_att,att_mask_input)
                train_err+=err
                train_acc+=acc
                train_batches += 1
...
#        test_predicted_classi_all=np.array([0])
        for batch in pro_data.iterate_minibatches(testing_word_pos_vec3D, \
          testing_label_1hot, testing_sen_length, model.batch_size, entity_pair_test3D, entity_test_len, shuffle=False):
            inputs, targets, mask_sen_length, input_att, input_att_mask_gru = batch
            mark_input=model.mask(mask_sen_length,model.batch_size)
            #attention mask gru(when=="1",and the gru can get through)
            att_mask_input=model.mask_att(input_att_mask_gru,model.batch_size)

            err, acc, test_predicted_classid = val_fn(inputs, targets, mark_input, model.negative_loss_alpha, model.negative_loss_lamda,input_att,att_mask_input)
            test_err += err
            test_acc += acc
            test_batches += 1
...
#        test_predicted_classid=np.array(test_predicted_classi_all[1:])
        ...
        att_mask_input=model.mask_att(entity_test_len,len(entity_test_len))
        err, acc, test_predicted_classid=val_fn(testing_word_pos_vec3D, testing_label_1hot, mark_input, model.negative_loss_alpha, model.negative_loss_lamda, entity_pair_test3D, att_mask_input)
        #confusion matrix
