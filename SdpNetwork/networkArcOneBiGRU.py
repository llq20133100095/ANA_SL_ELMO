...
# -*- coding: utf-8 -*-
"""
Created on Fri Feb  2 19:40:23 2018

Arc_One CNN model+Gru
...
from CustomLayers import SplitInLeft,SplitInRight,SplitInGlobal,HighwayNetwork1D,HighwayNetwork2D,DeleteFirstInCNN
...
class Network:
    ...
    def __init__(self):
        ...
        #input shape
        self.input_shape=(None,105,340)
        #mask shape
        self.mask_shape=(None,105)
        ##save ACCURACY picture path
        self.save_picAcc_path="../result/test-accuracy.jpg"
    ...
    def bulit_gru(self,input_var=None,mask_var=None,sen_length=None):
        ...
        """Convolutional layer in fliter_size 3"""
        # Convolutional layer in global SDP:
        #    fliter_size(3,cnn_gru_size)
        l_global_sdp_cnn_3f = lasagne.layers.Conv2DLayer(
            l_split_global_sdp, num_filters=100, filter_size=(3, self.cnn_gru_size),
            nonlinearity=lasagne.nonlinearities.rectify,
            W=lasagne.init.GlorotUniform())
    
        # Max-pooling layer of factor 2 in both dimensions:
        l_global_sdp_pooling_3f = lasagne.layers.MaxPool2DLayer(l_global_sdp_cnn_3f, pool_size=(2, 1))
        
        # Convolutional layer in global SDP
        l_global_sdp_cnn_3f = lasagne.layers.Conv2DLayer(
            l_global_sdp_pooling_3f, num_filters=100, filter_size=(3, 1),
            nonlinearity=lasagne.nonlinearities.rectify,
            W=lasagne.init.GlorotUniform())
        ...
        """Convolutional layer in foreward fliter_size 2"""
        # Convolutional layer in global SDP:
        #    fliter_size(3,cnn_gru_size)
        l_cnn_fore_2f = lasagne.layers.Conv2DLayer(
            l_split_global_sdp, num_filters=100, filter_size=(2, self.cnn_gru_size),
            nonlinearity=lasagne.nonlinearities.rectify,
            W=lasagne.init.GlorotUniform())
    
        # Max-pooling layer of factor 2 in both dimensions:
        l_pooling_fore_2f = lasagne.layers.MaxPool2DLayer(l_cnn_fore_2f, pool_size=(2, 1))
        
        # Convolutional layer in global SDP
        l_cnn_fore_2f = lasagne.layers.Conv2DLayer(
            l_pooling_fore_2f, num_filters=100, filter_size=(2, 1),
            nonlinearity=lasagne.nonlinearities.rectify,
            W=lasagne.init.GlorotUniform())
        ...
        # Convolutional layer in global SDP:
        #    fliter_size(3,cnn_gru_size)
        l_sdp_back=lasagne.layers.ReshapeLayer(
            DeleteFirstInCNN(l_split_global_sdp,size=self.num_steps-1),
            ([0],1,[1],[2]))
            
        l_cnn_back_2f = lasagne.layers.Conv2DLayer(
            l_sdp_back, num_filters=100, filter_size=(2, self.cnn_gru_size),
            nonlinearity=lasagne.nonlinearities.rectify,
            W=lasagne.init.GlorotUniform())
    
        # Max-pooling layer of factor 2 in both dimensions:
        l_pooling_back_2f = lasagne.layers.MaxPool2DLayer(l_cnn_back_2f, pool_size=(2, 1))
        
        # Convolutional layer in global SDP
        l_cnn_back_2f = lasagne.layers.Conv2DLayer(
            l_pooling_back_2f, num_filters=100, filter_size=(2, 1),
            nonlinearity=lasagne.nonlinearities.rectify,
            W=lasagne.init.GlorotUniform())
        ...
        """
        #Concatenate
        l_con=lasagne.layers.ReshapeLayer(
              lasagne.layers.ConcatLayer((l_global_sdp_pooling_3f,l_pooling_fore_2f,l_pooling_back_2f),axis=2),
              ([0],100,74))
        
        #output dropout
        l_con_drop=lasagne.layers.DropoutLayer(l_con,p=self.keep_prob_cnn)
        
        # A fully-connected layer of 200 units with 50% dropout on its inputs:
        l_con_den = lasagne.layers.DenseLayer(
            l_con_drop,
            num_units=150,
            nonlinearity=lasagne.nonlinearities.rectify)
                    
        l_out = lasagne.layers.DenseLayer(\
            lasagne.layers.dropout(l_con_den, p=self.keep_prob_cnn_gur_output),\
            num_units=self.num_classes,\
            nonlinearity=lasagne.nonlinearities.softmax)

        
        return l_out,l_gru_forward
...
if __name__=="__main__":
    ...
    """
    model=Network()
    gru_network,l_gru_forward=model.bulit_gru(input_var,mask_var,sen_length)
    
    # Create a loss expression for training, i.e., a scalar objective we want
    # to minimize (for our multi-class problem, it is the cross-entropy loss):
    prediction = lasagne.layers.get_output(gru_network)
    l_gru_forward=lasagne.layers.get_output(l_gru_forward)
    loss = lasagne.objectives.categorical_crossentropy(prediction, target_var)
    loss = loss.mean()
    
    #regularization:L1,L2
#    l1_penalty = lasagne.regularization.regularize_network_params(gru_network, lasagne.regularization.l1) * 1e-4
    l2_penalty = lasagne.regularization.regularize_network_params(gru_network, lasagne.regularization.l2) * 1e-4
    ...
    train_fn = theano.function([input_var, target_var, mask_var, learning_rate_var, adam_beta1_var, sen_length], [loss,train_acc,l_gru_forward], updates=updates)

    # Compile a second function computing the validation loss and accuracy and F1-score:
    val_fn = theano.function([input_var, target_var, mask_var, sen_length], [test_loss, test_acc, test_predicted_classid])
    ...
    for epoch in range(model.num_epochs):
        ...
        for batch in pro_data.iterate_minibatches(training_word_pos_vec3D, \
          training_label, training_sen_length, model.batch_size, shuffle=True):
            inputs, targets, mask_sen_length = batch
            mark_input=model.mask(mask_sen_length,model.batch_size)
            err ,acc, l_gru_forward = train_fn(inputs, targets, mark_input, learning_rate, adam_beta1, np.reshape(mask_sen_length,(-1,1)))
            train_err+=err
            train_acc+=acc
            train_batches += 1
        ...
        for batch in pro_data.iterate_minibatches(testing_word_pos_vec3D, \
          testing_label, testing_sen_length, model.batch_size, shuffle=False):
            inputs, targets, mask_sen_length = batch
            mark_input=model.mask(mask_sen_length,model.batch_size)
            err, acc, test_predicted_classid = val_fn(inputs, targets, mark_input, np.reshape(mask_sen_length,(-1,1)))
            test_err += err
            test_acc += acc
            test_batches += 1
        ...
        err, acc, test_predicted_classid=val_fn(testing_word_pos_vec3D, testing_label, mark_input, np.reshape(testing_sen_length,(-1,1)))
        #confusion matrix
    ...
    np.savez('../model/model.npz', *lasagne.layers.get_all_param_values(gru_network))
    
    model_save=Network()
    gru_network_save=model_save.bulit_gru(input_var)
    ...
    test_batches = 0
    for batch in pro_data.iterate_minibatches(testing_word_pos_vec3D, \
      testing_label, testing_sen_length, model_save.batch_size, shuffle=False):
        inputs, targets, mask_sen_length = batch
        mark_input=model_save.mask(mask_sen_length,model.batch_size)
        err, acc, test_predicted_classid = val_fn(inputs, targets, mark_input, np.reshape(mask_sen_length,(-1,1)))
        test_err += err
        test_acc += acc
        test_batches += 1
    ...
    #F1 value
    mark_input=model_save.mask(testing_sen_length,len(testing_sen_length))
    err, acc, test_predicted_classid=val_fn(testing_word_pos_vec3D, testing_label, mark_input, np.reshape(testing_sen_length,(-1,1)))
    #confusion matrix
    test_con_mat=confusion_matrix(testing_label,test_predicted_classid)
    #support
    _,_,_,support=precision_recall_fscore_support(testing_label,test_predicted_classid)   
    #computer F1_Score
    test_f1_score=model_save.accuracy_f1(test_con_mat,support)
    ...
    8.picture display
    """
    num_epochs=range(model_save.num_epochs)
    num_epochs=[i+1 for i in num_epochs]
    #save Accuracy picture(train and test)
    model_save.save_plt(x=num_epochs,y=test_acc_listplt,label='test accuracy',title="Train And Test Accuracy",ylabel_name="accuracy(%)",save_path=model_save.save_picAcc_path,twice=True,x2=num_epochs,y2=train_acc_listplt,label2='train accuracy')
    #save F1-SCORE picture 
    model_save.save_plt(num_epochs,f1_score_listplt,'f1-score',"F1-SCORE","f1-score(%)",model_save.save_picF1_path)
    #save loss picture(train)
    model_save.save_plt(num_epochs,train_loss_listplt,'train loss',"Train Loss","loss",model_save.save_lossTrain_path)
    #save loss picture(test)
    model_save.save_plt(num_epochs,test_loss_listplt,'test loss',"Test Loss","loss",model_save.save_lossTest_path)
...
#    att1 = MIMEText(open(model_save.save_picAcc_path, 'rb').read(), 'base64', 'utf-8')
...
#    att1 = MIMEText(open(model_save.save_picF1_path, 'rb').read(), 'base64', 'utf-8')
#    att1["Content-Type"] = 'application/octet-stream'
#    # filename in email display
