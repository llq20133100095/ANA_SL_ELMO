...
#            nonlinearity=lasagne.nonlinearities.softmax)
        ...
        return l_out_margin,l_in,l_mask,l_merge_drop,l_out_margin
...
#    test_loss=lasagne.objectives.categorical_crossentropy(test_prediction, target_var)
    ...
    if model.network_type=="pi":
        ...
        train_fn = theano.function([input_var, target_var, mask_var, learning_rate_var, adam_beta1_var, negative_loss_alpha, negative_loss_lamda], [loss,train_acc,l_gru,l_split], updates=updates, on_unused_input='warn')
        ...
        #Tempens model:run batch
            ...
            for batch in pro_data.iterate_minibatches(training_word_pos_vec3D, \
              training_label_1hot, training_sen_length, model.batch_size, shuffle=True):
                ...
                aa_mark_input=model.mask(mask_sen_length,model.batch_size)
                err ,acc, aa_l_gru,aa_l_split = train_fn(aa_inputs, targets, aa_mark_input, learning_rate, adam_beta1, model.negative_loss_alpha, model.negative_loss_lamda)
                train_err+=err
                train_acc+=acc
                train_batches += 1
